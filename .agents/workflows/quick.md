...
- `.ai-os-project/verification-matrix.yaml`：作为本次改动验证动作的判断基线（若已存在）
...
2. 在 `.ai-os-project/tasks.yaml` 中追加一条 quick task：
   ...
   - id: QUICK-NNN
     ...
     evidence_required:
       - "编译通过"
   ```
3. 读取 `.ai-os-project/verification-matrix.yaml`，或使用 `create-ai-os affected --dry-run` 判断本次改动需要的验证动作
4. 执行开发
5. 执行编译验证 + 运行相关测试
...
9. 更新 `.ai-os-project/tasks.yaml` 状态为 `done`，附简要证据
10. 更新 `.ai-os-project/STATE.md` 的进度概览
...
3. **共享代码的回归检查必须做**
4. **`.ai-os-project/tasks.yaml` 必须更新**（记录做了什么）
5. **git 提交必须规范**
