...
- `.ai-os-project/tasks.yaml`、`.ai-os-project/acceptance.yaml`、`.ai-os-project/STATE.md` 任一缺失
- 当前阶段仍在需求澄清或 spec 未确认
- 任务包含高风险写操作且尚未拿到人工确认

## 会生成或更新什么

- `.ai-os-project/tasks.yaml`：推进任务状态、补充证据
- `.ai-os-project/verification-matrix.yaml`：作为变更感知验证的基线输入
- `.ai-os-project/STATE.md`：同步当前位置、最近决策、下一步
...
- `.ai-os-project/tasks.yaml` 已定义且经用户确认
- `.ai-os-project/acceptance.yaml` 已定义
- `.ai-os-project/STATE.md` 存在且当前位置清晰
- 用户**明确授权**自动推进（不可自行假设）

## 执行循环

按 `.ai-os-project/tasks.yaml` 中的 wave 顺序，对每个 `status: todo` 且 `depends_on` 全部完成的任务：

1. **前置检查**：
   - 读取 `.ai-os-project/STATE.md` 确认全局方位
   - 检查该任务的 Definition of Ready 是否满足
   - 读取该任务的 `context_files`
   - 读取 `.ai-os-project/memory.md` 获取长期约束
   - 读取 `.ai-os-project/verification-matrix.yaml`，确认当前任务的 `verification_required`
2. **执行任务**：
   - 编码实现
   - 编译验证
...
3. **后置更新**：
   - 更新 `.ai-os-project/tasks.yaml` 状态和证据
   - 更新 `.ai-os-project/STATE.md` 的当前位置和进度
...
- [ ] 实现是否偏离 spec
- [ ] 是否有新发现需要记录到 `.ai-os-project/STATE.md` 或 `.ai-os-project/memory.md`
...
1. 确认 `.ai-os-project/STATE.md` 记录了最后完成的任务
2. 确认 `.ai-os-project/tasks.yaml` 中已完成任务的状态已更新
3. 用户再次说"继续"或"auto"时，从 `.ai-os-project/STATE.md` 恢复位置继续
