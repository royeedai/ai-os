...
assert(tasksTemplate.includes("parity_checks:"), "tasks template includes parity checks");
assert(tasksTemplate.includes("similar_impl_refs:"), "tasks template includes same-repo implementation refs");
assert(tasksTemplate.includes("step_validation:"), "tasks template includes step validation plan");
...
assert(acceptanceTemplate.includes("shared-impact-check"), "acceptance template includes shared impact evidence");
assert(acceptanceTemplate.includes("route-contract-check"), "acceptance template includes route contract evidence");
assert(acceptanceTemplate.includes("schema-parity-check"), "acceptance template includes schema parity evidence");
assert(acceptanceTemplate.includes("state-triage-note"), "acceptance template includes code/data/runtime triage evidence");
...
assert(specTemplate.includes("**共享层 / 包装层副作用审计**"), "spec template includes shared-layer side-effect field");
assert(specTemplate.includes("**路由 / 入口契约对照**"), "spec template includes route contract field");
assert(specTemplate.includes("**Schema / 存储一致性说明**"), "spec template includes schema consistency field");
assert(specTemplate.includes("**同仓正常实现对照**"), "spec template includes same-repo reference field");
assert(specTemplate.includes("**异常/空数据证据**"), "spec template includes degraded-path evidence");
assert(specTemplate.includes("**最小验证步骤**"), "spec template includes step validation field");
