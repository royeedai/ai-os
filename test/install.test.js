...

{
  ...
  assert(localDoctorVersion && localDoctorVersion.trim() === "10.3.1", "local doctor VERSION matches framework version");
  ...
  assert(toml && toml.includes('framework_version = "10.3.1"'), "framework.toml has version 10.3.1");
...

{
  ...
  assert(result.stdout.trim() === "10.3.1", `--version outputs 10.3.1 (got ${result.stdout.trim()})`);
}
