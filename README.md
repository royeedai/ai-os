...
| 你现在的场景 | 先用什么 | 结果 |
|------|------|------|
| 老项目第一次接入 AI-OS | `npx create-ai-os .` → `/init` | 安装框架后用 `/init` 一键生成项目基础文件 |
...
| 阶段 | 记住这些入口 | 作用 |
|------|------|------|
| Start | `/init`、`/new-project`、`/map-codebase`、`/quick`、`/clone-project` | 先把任务送进正确 workflow |
...
安装完成后，在 AI 工具中使用 `/init` 工作流，AI 会分析现有代码库并自动生成项目章程、任务图、STATE 等基础文件。

参数说明：

- `my-project`：目标项目目录
- `--with-project-files`：额外生成项目章程、任务、验收、发布、记忆、验证基线等初始文件（空模板）
- `.`：在当前已有仓库中安装 AI-OS 框架文件，已有文件不会被覆盖
...
对于老项目，运行 `npx create-ai-os .` 只会安装框架受管文件和元数据。项目状态文件可通过 `/init` 工作流一键生成（基于现有代码分析填入真实内容），或在使用其他 workflow 时按需自动创建。

当仓库还没有接入 AI-OS 时，先运行 `npx create-ai-os .` 安装框架，再使用 `/init` 或其他 workflow。
...
2. 在 AI 工具里使用 `/init` 初始化项目基础文件

`/init` 会分析现有代码库，自动生成项目章程、任务图、STATE 等基础文件，内容基于代码分析填入真实数据，而非空模板。

初始化完成后，按场景选择后续 workflow：
