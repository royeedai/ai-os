...
先不要记所有 skill 名称。第一次上手只需要分清 3 条入口：

| 你现在的场景 | 先用什么 | 结果 |
|------|------|------|
| 从 0 开始做一个项目 | `/new-project` | 先得到项目章程、任务图和风险边界 |
| 已有仓库上加功能 | `/map-codebase` → `/new-module` | 先摸清现有架构，再进入模块闭环 |
...
| 自动连续推进 | `/auto-advance` | 在 `.ai-os-project/STATE.md` / `.ai-os-project/tasks.yaml` 缺失时继续 |
| 看当前进度 | `/status` | 只靠聊天上下文回忆 |
| 判断下一步 | `/next` | 不看依赖图和 wave 就拍脑袋选任务 |
| 恢复中断上下文 | `/resume` | 不读 `.ai-os-project/STATE.md` 就直接接着做 |
...
- 补全 `.ai-os-project/project-charter.md`、`.ai-os-project/risk-register.md`、`.ai-os-project/STATE.md`

3. 若在已有代码库上开发，先走 `/map-codebase`

- 分析技术栈、架构、命名约定、现有模式
- 产出 `.ai-os-project/codebase-map.md`，后续开发参考

4. 每做一个模块走 `/new-module`

- 阶段零：需求澄清（Discuss），理解用户意图
- 阶段一：需求定义，产出 `.ai-os-project/specs/*.spec.md`
...
- 仍需更新 `.ai-os-project/tasks.yaml` 和保证编译

6. 想让 AI 连续执行时走 `/auto-advance`

- 按 `.ai-os-project/tasks.yaml` 的 wave 顺序自动推进
...
- 完成 `.ai-os-project/release-plan.md`
...
- 把重复性坑和有效做法沉淀进 `.ai-os-project/memory.md`
- 把关键回归场景沉淀进 `.ai-os-project/evals/`
...
2. 收集素材
   ...
   - 在项目里创建 `.ai-os-project/references/clone-materials/` 目录
...
- `.ai-os-project/project-charter.md`：项目章程、范围边界、成功标准、非功能要求
- `.ai-os-project/specs/*.spec.md`：模块需求规格
- `.ai-os-project/specs/*.context.md`：模块需求澄清记录（Discuss 阶段产出）
- `.ai-os-project/tasks.yaml`：任务图、依赖、wave 并行标注、DoR/DoD、证据要求
- `.ai-os-project/acceptance.yaml`：验收门禁和 Evidence Pack
...
除了 `create-ai-os` 初始化命令外，还提供一组项目生命周期管理命令。
...
输出分类：`modified`（本地改动冲突）、`outdated`（仅版本落后，可安全升级）、`missing`（母仓库新增）、`extra`（项目独有）、`unchanged`（一致）。
...
升级只更新框架受管文件（`AGENTS.md`、`.agents/`），不动项目自有文件（`.ai-os-project/specs/`、`.ai-os-project/tasks.yaml`、`.ai-os-project/memory.md` 等）。用户自定义的额外 skill 或 workflow 会被保留。

注意：

- `--preflight` 可先检查是否存在需人工确认的覆盖冲突
- `--force` 只应该用于替换 `AGENTS.md` 和 `.agents/`
- 项目自己的 `.ai-os-project/specs/`、`.ai-os-project/tasks.yaml`、`.ai-os-project/memory.md` 不应该被母仓库覆盖
...
检查 `.ai-os-project/release-plan.md` 是否具备具体的发布步骤、Smoke Check、回滚条件，并交叉验证 `.ai-os-project/acceptance.yaml` 和 `.ai-os-project/tasks.yaml`。
...
node ./bin/ai-os-validate.js my-project
node ./bin/ai-os-diff.js my-project
node ./bin/ai-os-upgrade.js my-project
node ./bin/ai-os-status.js my-project
node ./bin/ai-os-next.js my-project
node ./bin/ai-os-resume.js my-project
node ./bin/ai-os-release-check.js my-project
```
