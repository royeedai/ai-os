...
- **当前覆盖锚点**：`framework/.agents/templates/project/MISSION.md` 的单 `当前基线 ID` 结构、`framework/AGENTS.md` 与 `README.md` 中“串行基线、并行实现”的协作规则、`framework/.agents/workflows/AGENTS.md` / `align.md` / `change-request.md` / `build.md` / `verify.md` / `ship.md` 的 lane 进入与作用域规则、`framework/.agents/templates/lane/lane.toml` 的 lane 元数据模板、`bin/shared.js` 的 lane 解析、lane 元数据读取、作用域提示与 worktree 启发式、`bin/ai-os-lane.js` 的 lane 生命周期命令与 topology / metadata 提示、`bin/ai-os-status.js` / `bin/ai-os-doctor.js` 的 lane 元数据摘要、`bin/ai-os-validate.js` / `bin/ai-os-gate.js` / `bin/ai-os-release-check.js` 的 lane-aware 修复建议与作用域提示、`docs/cli.md`、`docs/workflows.md` 与 `README.md` 的 lane 管理说明、`docs/evolution/multi-delivery-lanes-proposal.md`、`examples/quickstart-todo-cli/` 的 `shared root + lanes/default` canonical example、`examples/legacy-to-lanes-migration.md`、`evals/legacy-to-lanes-migration-skipped.md`、`test/lane.test.js`、`test/install.test.js`、`test/team.test.js`、`test/unit-shared.test.js`
- **每次迭代核对**：不能把 Git 冲突缓解误当成多迭代并行已经被支持；不能在未重构工件拓扑前暗示 AI-OS 已支持“单工作区多当前基线”；若进入 lane 模型，必须同步清理单当前交付的默认假设和相关 CLI 校验。

### PG-007 lane 归档只改状态，没有把稳定经验回流到共享记忆

- **来源**：2026-04-16 `7.2` 负责人收口评审；lane 生命周期命令可用后，发现 `archive` 仍可能退化成“只改 status = archived”
- **真实问题**：团队并行交付时，一条 lane 做完后往往沉淀了新的稳定约束、坑点或代码模式。如果 `lane archive` 只是把 lane 标成 `archived`，却没有明确这些结论是否已经同步到共享 `.ai-os/memory.md` / `.ai-os/CONVENTIONS.md`，那么经验会继续留在 lane 私有 `MISSION.md` / `DESIGN.md` / `STATE.md` 中；后续 lane 无法直接恢复这些结论，团队又会重复踩坑。维护 AI-OS 本身时，还会额外出现根层 `docs/problem-ledger.md` 未同步的问题。
