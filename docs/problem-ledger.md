...
- **当前覆盖锚点**：`framework/.agents/templates/project/MISSION.md` 的单 `当前基线 ID` 结构、`framework/AGENTS.md` 与 `README.md` 中“串行基线、并行实现”的协作规则、`framework/.agents/workflows/AGENTS.md` / `align.md` / `change-request.md` / `verify.md` 的 lane 进入与作用域规则、`bin/ai-os-validate.js` 对 Mission / tasks / acceptance 单 baseline_id 一致性的校验、`bin/ai-os-lane.js` 的 lane 生命周期命令（`list/add/activate/archive`）、`docs/cli.md` 与 `README.md` 的 lane 管理说明、`docs/evolution/multi-delivery-lanes-proposal.md`、`examples/quickstart-todo-cli/` 的 `shared root + lanes/default` canonical example、`examples/legacy-to-lanes-migration.md`、`evals/legacy-to-lanes-migration-skipped.md`、`test/lane.test.js`、`test/install.test.js`
