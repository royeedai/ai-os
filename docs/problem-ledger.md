...
- **当前覆盖锚点**：`framework/.agents/templates/project/MISSION.md` 的单 `当前基线 ID` 结构、`framework/AGENTS.md` 与 `README.md` 中“串行基线、并行实现”的协作规则、`bin/ai-os-validate.js` 对 Mission / tasks / acceptance 单 baseline_id 一致性的校验、`docs/evolution/multi-delivery-lanes-proposal.md`、`examples/quickstart-todo-cli/` 的 `shared root + lanes/default` canonical example、`examples/legacy-to-lanes-migration.md`、`evals/legacy-to-lanes-migration-skipped.md`
