...
Pin a release for reproducible, cache-friendly installs: `npx --yes github:royeedai/ai-os#v10.5.0 .`. The install is the only step that needs the network.
...
The legacy remote form `npx --yes github:royeedai/ai-os#v10.5.0 doctor .` still works for a one-time audit of a project you have not installed into; it is not for daily use.
