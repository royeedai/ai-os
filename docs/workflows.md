...
| `/map-codebase` | 首次接手或范围不清时先理解现有代码库 | 识别技术栈、架构、命名和约束 |
...
1. `/init`
2. `/new-module` 或 `/quick`
3. `/review`

这里最重要的是，先让 AI 认识现有代码库，再开始动手。

但这不等于“以后每次改动都先 `/map-codebase`”。

如果用户已经明确点名现有模块、接口、字段或命令，例如“给用户模块加一个次数限制”，应先按范围在 `/quick` 和 `/new-module` 之间选择，再做定点阅读；只有当局部阅读仍不足以判断结构和影响边界时，才升级到 `/map-codebase`。
...
如果 `/quick` 只是纯视觉微调，也可以建议切到静态 UI 友好模型处理；但一旦涉及交互、状态、接口或数据逻辑，就不能继续按“纯静态 UI”处理。

## `/new-module` 现在不是固定重流程

`/new-module` 现在会先做 5 个判断：

- 模块类型：`页面类` / `API 类` / `数据处理类` / `工具类`
- 交付等级：`L1` / `L2` / `L3`
- 是否属于当前里程碑目标
- 是否依赖尚未具备的共享基础能力
- 若为页面类，是否存在可以独立切分的静态 UI 子任务
...
如果 `页面类` 模块里有可以独立切分的静态 UI 子任务，允许建议切到更适合视觉产出的模型或新 session 先完成静态结构、样式和响应式，再回到主流程做真实数据接入、状态管理和验收收口。AI-OS 只负责交接规范，不负责自动切换模型。
