...
| D03-R02 | Distinguish development VERSION from latest RELEASED_VERSION | `node --test test/release-truth.test.js` | VERSION is 11.0.0 and RELEASED_VERSION is 10.5.1 | Release-truth suite passes | pass |
| D03-R03 | Pin user docs to RELEASED_VERSION, keep changelog unreleased, and do not install release metadata | `node --test test/release-truth.test.js test/package.test.js` | Pins are truthful and downstream layout excludes RELEASED_VERSION | Tests pass | pass |
| D03-R04 | Use explicit pinned GitHub invocations and never official bare npx create-ai-os | `node --test test/release-truth.test.js test/docs.test.js` | No unsupported package invocation is advertised | Scans pass | pass |
...
| D04-R01 | Framework-owned paths replace only after safety checks and record source hashes | `node --test test/install-plan.test.js test/install-transaction.test.js test/manifest.test.js` | Framework mutations are planned, safe, and hashed | Installer suites pass | pass |
...
| D04-R05 | Team config changes only one strict managed block | `node --test test/team-config.test.js test/migration.test.js test/doctor-readiness.test.js` | Outside bytes are unchanged and malformed blocks fail | Team-config suites pass | pass |
| D05-R01 | Install the exact canonical layout schema 11 | `node --test test/install.test.js test/manifest.test.js` | Fresh install file set and metadata equal layout 11 | Layout suites pass | pass |
...
| D05-R04 | Commit stable metadata without timestamps and ignore only session or temporary state | `node --test test/manifest.test.js test/install.test.js test/doctor-readiness.test.js` | Reproducible metadata and exact ignore rules | Tests pass | pass |
| D05-R05 | managed-files.tsv has exact columns and hashes only framework sources | `node --test test/manifest.test.js test/doctor-layout.test.js` | Manifest syntax, inventory, and hashes are exact | Manifest tests pass | pass |
| D05-R06 | Treat managed-files.tsv as the sole non-self-listed exception | `node --test test/manifest.test.js test/doctor-layout.test.js` | Every other managed path appears exactly once | Completeness tests pass | pass |
...
| D06-R05 | Classify every destination by ownership and current hash | `node --test test/install-plan.test.js test/manifest.test.js` | Operation plan has deterministic ownership decisions | Planning tests pass | pass |
| D06-R06 | Detect foreign AGENTS, IDE pointers, and team-config conflicts | `node --test test/install-idempotency.test.js test/team-config.test.js` | Conflicts are reported before writes | Conflict tests pass | pass |
...
| D07-R01 | Recognize v10 metadata, all distributed tag hashes, and legacy managed blocks | `node --test test/compat-manifest.test.js test/migration.test.js` | Compatibility catalog is exact and version-scoped | Compatibility tests pass | pass |
...
| D07-R06 | Replace legacy gitignore block so only STATE remains ignored | `node --test test/migration.test.js test/team-config.test.js` | Stable metadata is committed and STATE ignored | Managed-block tests pass | pass |
| D07-R07 | Remove the managed memory merge=union rule | `node --test test/migration.test.js test/team-config.test.js` | No active AI-OS union merge rule remains | Migration tests pass | pass |
...
| D07-R09 | Stop before writes for customized constitution or team configuration | `node --test test/migration.test.js test/team-config.test.js` | Precise conflict and unchanged snapshot | Conflict tests pass | pass |
...
| D12-R01 | Remove memory merge=union and use ordinary Git conflict handling | `node --test test/migration.test.js test/team-config.test.js` | No managed union rule remains | Tests pass | pass |
...
| D14-R09 | Cover release truth separation | `node --test test/release-truth.test.js` | VERSION and RELEASED_VERSION semantics hold | Release suite passes | pass |
...
| D15-R07 | Cover Unicode/space paths and CRLF | `node --test test/path-safety.test.js test/team-config.test.js test/ci-config.test.js` | Cross-platform path/text cases exist | Tests pass | pass |
...
| D16-R01 | Require VERSION, package, lockfile, changelog target, and candidate tag agreement for every release | `node --test test/release-truth.test.js test/docs.test.js` | Seven-claim checklist includes version agreement | Release policy tests pass | pass |
| D16-R02 | Require docs pins to agree with RELEASED_VERSION before and after transition | `node --test test/release-truth.test.js` | Pin transition is explicit | Release truth tests pass | pass |
...
| D16-R08 | Keep public tag/release separate and never advertise nonexistent v11 | `node --test test/release-truth.test.js test/docs.test.js` | v11 stays unreleased until separately authorized | Release truth tests pass | pass |
...
| D18-R01 | Ensure no official command references a nonexistent or unintended package/ref | `node --test test/release-truth.test.js test/docs.test.js` | All public commands are pinned to real release metadata | Command scans pass | pass |
...
| D18-R11 | Pass native tests, lint, diff, pack/tarball, coverage, and supported-platform CI | `npm test`; `npm run test:coverage`; `npm run lint`; `git diff --check`; `node scripts/verify-remote-evidence.js` | Local gates and nine current-head remote checks pass | Local gates pass; branch/PR CI not yet available | blocked |
| D18-R12 | Apply repository security/release settings or prove feature unavailability without misleading claims | `node scripts/verify-repository-settings.js` | Current live settings satisfy desired state | GitHub authentication is invalid; no live claim made | blocked |
...
- Repository data/settings: committed desired state exists; live GitHub mutation/readback is blocked until GitHub CLI authentication is restored.
