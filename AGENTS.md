...
- 仓库根目录下的 `PROJECT_PURPOSE.md` 是本仓库产品目的、目标用户、决策原则和方向优先级的单一真相源
- 当用户提出产品想法、README 改造、workflow 调整、模板变更、定位变化时，必须先对齐 `PROJECT_PURPOSE.md`
- 如果一个方案只修局部症状、但与项目目的或方向冲突，应明确指出冲突并优先回到目的层重新规划
- 纯局部 bug 修复可以直接处理；系统性问题应按“方向层 -> 体验层 -> 框架层 -> 实现层”规划
- 不要把用户每次提出的问题都直接编码成 AI-OS 框架规则；先判断这个问题究竟属于 AI-OS 的产品目标，还是某个项目的使用问题
- 承接位置的判断不是固定顺序，而是目标驱动：根据问题性质，决定应落在 `framework/`、项目级工件、文档/示例，或明确不纳入
- 进入 `framework/` 的改动，至少要满足：高频复现、可通用抽象、长期收益明显大于新增复杂度
- 即使决定进入 `framework/`，也不要默认以“新增一条”的方式处理；应先审视现有结构，优先合并、重写、抽象或替换，避免规则层持续增厚
- 如果一个需求更适合通过使用方式、项目章程、spec、tasks 或示例解决，应直接说明“不建议加到 AI-OS 框架”，并给出替代做法
- 新规则如与现有规则重叠，优先合并、抽象或删除旧规则，避免持续叠加特例
