...
## 二⅘、体验与视觉必须匹配目标市场 (Market-Fit UX)

**用户可见的设计、布局、交互和文案，不得直接套用 AI 的默认审美，必须优先匹配目标市场和目标用户习惯。**

- 项目章程和 spec 中必须明确目标市场 / 主要使用地区，以及是否存在既有设计系统或明确参考方向。
- 若项目主要面向中国大陆用户，默认采用国内主流产品习惯：更贴近实际业务的信息密度、导航结构、表格与筛选布局、表单组织方式、操作路径和文案语气。
- 若项目已有设计系统、品牌规范或目标市场不是中国大陆，则以既有体系和实际目标市场为准，不得强行套用“国内风格”。
- 在目标市场不明确时，必须先确认或在工件中写明默认假设；不允许静默产出偏欧美 SaaS 的低密度通用界面。

## 二⅚、共享基础能力先于业务模块 (Foundation Before Modules)

**对于存在共享基础能力依赖的平台型、后台型、流程型项目，不得按模块列表顺序机械推进。**

- 若多个业务模块共同依赖登录、权限、多语言、导航骨架、组织/工作区/仓库上下文、基础设置、数据字典等平台能力，必须先把这些依赖纳入规划与任务图，再推进业务模块。
- 第一阶段目标不得只是“完成第一个模块页面”，而应优先达到“基础能力可用 + 首条核心业务闭环可运行”。
- 在依赖未具备时，不得把业务模块标记为正式进入实现；应先补齐基础能力任务，或把该模块回退为待排期状态。
- 阶段验收不得接受“孤立模块完成但整体不可运行”的伪完成状态。
- WMS、ERP、OMS、多租户 SaaS 后台、管理系统等都属于常见示例，但规则判断依据应是共享依赖结构，而不是行业名称本身。
