...
- 项目必须维护 `.ai-os/STATE.md`（后续简称 `STATE.md`），记录当前位置（里程碑/模块/阶段/任务）、进度概览、阻塞项、最近决策和下一步。
- 每次任务状态变更、模块完成、阶段切换时，必须同步更新 `STATE.md`。
- `STATE.md` 是"快速恢复方位"的唯一入口，禁止让 AI 通过读取所有工件文件来推断当前状态。

## 二¾、目的与当前里程碑优先 (Goal-Aligned Triage)

**接到 Bug、想法、优化建议时，不得脱离当前项目目的和里程碑直接开工。**

- 在执行修复、优化或临时想法前，必须先对齐 `.ai-os/project-charter.md` 和 `STATE.md`，判断该事项是否阻塞当前项目目标、当前里程碑或当前验收路径。
- 若事项直接阻塞当前核心目标或验收路径，应立即进入对应流程（如 `/debug`、`/incident`、`/change-request`）。
- 若事项不阻塞当前目标，且用户未明确调整优先级，应记录到 `tasks.yaml` / `STATE.md`，避免打断当前主线。
- 若事项反复出现，说明它不是单点问题，必须沉淀到 `memory`、`evals`，必要时补足 spec / task / acceptance / release 等上游工件。
- 不允许把所有问题都自动提升为当前最高优先级；优先级必须有明确的目标依据。
