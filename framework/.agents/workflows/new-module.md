...
当用户说“开发一个新模块”“加个 XXX 功能”“做一个 XXX 页面”时触发此流程。若用户其实是在现有模块上补一个局部能力，不要因为“加功能”三个字就默认转成全仓分析。

## 阶段零：入口判断

在进入正式流程前，先做意图与范围锚定：

- 若用户已明确指出现有模块、文件、接口、字段或规则，先定点读取相关代码、相关 spec 和 `.ai-os/STATE.md`
- 不要把“已有仓库上加一个局部能力”自动等同于“先做 `/map-codebase`”
- 只有在模块位置不清、现有模式不清、影响边界不清，或当前仓库确实缺少基础认知时，才补读 `.ai-os/codebase-map.md` 或升级到 `/map-codebase`
...
- `.ai-os/codebase-map.md`（仅在 brownfield 项目中，且局部阅读仍不足以判断结构或约束时）
- 当前模块直接相关的 spec、tasks、memory 和代码文件
