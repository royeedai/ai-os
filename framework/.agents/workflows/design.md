...
4. 确认本轮必须先锁的关键页面 / 关键流程 / 核心接口旅程 / 核心设计决策
5. 生成或更新 `.ai-os/DESIGN.md`：
   - 信息架构
   - 关键页面与交互
   - 关键流程
   - 视觉方向
   ...
   - 方案选型依据、核心约束、风险与注意事项
   - 设计确认记录
6. 若设计引入了 `/align` 中未覆盖的新依赖（框架、插件、SDK、基础镜像等），在锁定前对其执行实际可用性验证（包管理器查询、registry API、manifest 检查等），禁止仅凭 AI 训练数据认定版本存在
7. 如果是 `reverse-spec` 模式，生成或更新 `.ai-os/design-pack/parity-map.md`
8. 显式区分"必须用户确认的核心决策"和"AI 可自行处理的非核心细节"
9. 把已锁定内容、待确认项、确认停点写回 `.ai-os/STATE.md`
10. 向用户输出 `DESIGN.md` 核心内容、预设逻辑、共享基础设施基准、选型依据和待确认项，等待明确确认
11. 若关键设计仍未确认，暂停在此，不进入 `/plan` 或 `/build`
