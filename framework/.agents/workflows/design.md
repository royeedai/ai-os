...
5. 生成或更新 `.ai-os/DESIGN.md`：
   ...
   - 方案选型依据、核心约束、风险与注意事项
   - 设计确认记录
6. 如果是 `reverse-spec` 模式，生成或更新 `.ai-os/design-pack/parity-map.md`
7. 显式区分“必须用户确认的核心决策”和“AI 可自行处理的非核心细节”
8. 把已锁定内容、待确认项、确认停点写回 `.ai-os/STATE.md`
9. 向用户输出 `DESIGN.md` 核心内容、预设逻辑、共享基础设施基准、选型依据和待确认项，等待明确确认
10. 若关键设计仍未确认，暂停在此，不进入 `/plan` 或 `/build`
