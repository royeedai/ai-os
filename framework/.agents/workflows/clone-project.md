...
9. 所有产出必须以**当前项目**为主体命名和描述；参考对象只允许出现在"参考来源"一节，不得把 `legacy`、"原项目"、"兼容旧系统"、源系统品牌名等带入模块名、页面名、用户文案、验收项，除非用户明确要求迁移或兼容
10. 调用 `project-planner`，选择"复刻/仿制项目" archetype，生成 `.ai-os/project-charter.md`
11. 汇总所有 `[待确认]` 条目，向用户集中确认
12. 用户确认后，调用 `spec-validator` 验证每个 spec 的完整性
13. 调用 `task-orchestrator` 生成 `.ai-os/tasks.yaml`
14. 调用 `acceptance-gate` 生成 `.ai-os/acceptance.yaml`

> **门禁检查点**：阶段二产出（spec + tasks + acceptance）经用户确认后，方可进入阶段三。此时每个模块已具备完整的 spec、任务图和验收条件，不需要再重复创建。

## 阶段三：标准交付（与新项目合流）

15. 按 `.ai-os/tasks.yaml` 顺序，对每个模块走 `/new-module` 的**阶段二（技术设计）起**——跳过阶段零（需求澄清）和阶段一（需求定义），因为 spec、tasks、acceptance 已在本流程阶段二完成
16. 中途对方补充新截图/API 时，走 `/change-request` 同步更新 spec 和任务
17. 模块完成后走 `/review`
18. 准备交付时走 `/ship`
