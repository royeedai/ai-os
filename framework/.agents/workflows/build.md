...
5. **Wave 前置 -- 回归基线快照**：每个 wave 开始前，如果项目有测试套件，运行一次并记录当前通过的测试数量作为基线；如果有 lint / type-check，同时记录当前状态
6. 执行前加载任务的 `context_files`、`impact_tags` 和 `derived_checks`
7. 按 `execution_role` 选择当前任务的工作视角：
   - `design_mapper`
   - `contract_mapper`
   - `implementer`
   - `reviewer`
8. 命中 `approval_required` 时暂停，等人工确认
9. 实现时必须对照 spec 中的契约基准和字段映射 / 适配说明，避免跨层命名与状态枚举漂移
10. 实现时必须对照 `.ai-os/CONVENTIONS.md`（如存在）检查代码模式一致性，发现新代码引入了与约定不一致的模式时先修正再继续
11. 实现时严格锁定任务边界；不在任务范围内的优化、重构和顺手修补一律不做
12. 遇到需求矛盾、依赖缺失、方案失效、影响范围超出预期时，立即暂停并同步用户
13. 每个 wave 完成后、进入下一个 wave 前，执行 wave 自审检查：
   ...
   - **回归对比**：如果第 5 步记录了测试基线，重新运行测试套件并与基线对比；任何从"通过"变为"失败"的测试都视为回归，必须先修复回归再继续下一个 wave
...
14. 完成后回写当前 lane 的 `tasks.yaml`、`STATE.md` 和相关证据；`tasks.yaml` 只更新当前责任人任务的运行态字段（如 `status`、`blockers`、`notes`），不要顺手改 `baseline_id`、里程碑定义、他人任务或需求映射；如有测试基线，同步回写回归对比结论
15. 当任务由 IDE 计划模式、todo 列表或其他外部编排机制驱动时，所有编排任务完成后不等于交付完成；必须显式进入 `/verify` 执行项目原生静态校验和验收门禁，再进入 `/ship` 完成交付收口
