...
`/quick` 代表的是最低足够流程，不代表“可以不留痕、不验证、不更新状态”。
...
4. 明确告知这是一次“最低足够流程”的处理，而不是完整模块交付
5. 在 `.ai-os/tasks.yaml` 中追加一条 quick task：
   ```yaml
   - id: QUICK-NNN
     title: "简要描述"
     type: quick
     ...
     evidence_required:
       - "编译通过"
   ```
6. 读取 `.ai-os/verification-matrix.yaml`，或使用 `create-ai-os affected --dry-run` 判断本次改动需要的验证动作
7. 执行开发
8. 执行编译验证 + 运行相关测试
9. 若命中 `restart` / `cold-start-smoke`，必须补做受影响服务重启和冷启动 Smoke Check，并回填 `restart-log` / `cold-start-log` / `post-restart-smoke-log`
10. 若修改了共享代码（Model、工具函数、中间件、共享组件），必须做回归检查
11. 按 `git-workflow` 规范提交代码
12. 更新 `.ai-os/tasks.yaml` 状态为 `done`，附简要证据
13. 更新 `.ai-os/STATE.md` 的进度概览、最近决策和下一步
