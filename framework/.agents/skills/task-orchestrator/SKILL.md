...
9. 若项目存在共享基础能力依赖结构，必须先建立基础能力任务层与业务模块依赖图，再安排首条业务闭环
...
- **依赖优先**：对于存在共享基础能力依赖的项目，登录、权限、多语言、导航骨架、组织/工作区/仓库上下文、基础设置、数据字典等基础能力任务必须先于依赖它们的业务模块任务进入就绪状态
- **首条闭环优先**：第一阶段至少拆出一条从入口到结果展示的核心业务闭环任务链，不接受只拆孤立页面或孤立接口
